...
void channelScanHandleAck(const uint8_t *data, int len) {
    if (len == sizeof(AckPacket)) {
        ...
        if (ack->magic == 0xDEADBEEF && ack->status == 0) {
            ...
            Serial.printf("[SCAN] Sync ACK from Bridge for channel %d\n", ack->channel);
        }
    }
}
...
static bool sendSyncAndWaitAck(uint8_t newChannel) {
    ...
    Serial.printf("[SCAN] Bridge confirmed on channel %d\n", newChannel);
    return true;
}
...
uint8_t channelScanStart() {
    ...
    Serial.println("[SCAN] Waiting for the best channel from the Bridge...");
    ...
    Serial.printf("[SCAN] ACK sent to Bridge for channel %d\n", receivedBestChannel);
