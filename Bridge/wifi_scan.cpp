...
void printChannelResult(uint8_t channel) {
    Serial.printf("\n[Bridge] Channel %d completed: %d packets\n",
...
void wifiScanRecv(const esp_now_recv_info *info, const uint8_t *data, int len) {
    if (len == sizeof(BestChannelPacket)) {
        ...
        if (pkt->magic == 0xDEADBEEF && pkt->status == 0) {
            ...
            Serial.printf("[WIFI-Bridge] ACK received from Controller for channel %d\n", pkt->bestChannel);
        }
        return;
    }
...
void wifiScanLoop() {
    ...
    if (testComplete && !finalRankingPrinted) {
        ...
        if (bestChannelToSend > 0) {
            ...
            Serial.printf("[WIFI-Bridge] Best channel sent: %d\n", bestChannelToSend);
...
uint8_t wifiChannelSelectOrApply() {
    if (manualChannel > 0) {
        ...
        Serial.printf("[WIFI-Bridge] Using manual channel: %d\n", ch);
    ...
    if (manualChannel == 0) {
        Serial.println("[WIFI-Bridge] Using default channel: 1");
    ...
    if (savedChannel >= 1 && savedChannel <= 13) {
        Serial.printf("[WIFI-Bridge] Saved channel found: %d\n", savedChannel);
    ...
    if (!waitingMessagePrinted) {
        Serial.println("[WIFI-Bridge] Waiting for the scan from the Controller...");
    ...
    if (wifiScanComplete && bestChannelToSend > 0) {
        Serial.printf("[WIFI-Bridge] Channel received from Controller: %d\n", bestChannelToSend);
        esp_wifi_set_channel(bestChannelToSend, WIFI_SECOND_CHAN_NONE);
        bool isBroadcast = true;
        for (int i = 0; i < 6; i++) {
            if (peerMac[i] != 0xFF) {
    ...
    Serial.println("[WIFI-Bridge] Timeout or failure; using channel 1");
    esp_wifi_set_channel(1, WIFI_SECOND_CHAN_NONE);
    bool isBroadcast = true;
    for (int i = 0; i < 6; i++) {
        if (peerMac[i] != 0xFF) {
