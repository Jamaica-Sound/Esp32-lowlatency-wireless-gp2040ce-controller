...
static void applyManualChannel() {
    ...
    Serial.printf("[Bridge] Manual channel set to %d\n", manualChannel);
    ...
    if (!isZero && !isBroadcast) {
        ...
        addPeer(peerMac);
        Serial.printf("[Bridge] Peer updated to channel %d\n", manualChannel);
    }
}
...
void pairingBegin() {
    ...
    if (isManual) {
        ...
        if (!isBroadcast) {
            ...
            Serial.println("[Bridge] Broadcast MAC, skip addPeer");
        }

        paired = true;
        pairingState = STATE_IDLE;
        applyManualChannel();
        Serial.println("[Bridge] Manual MAC configured, pairing bypassed");
        return;
    }
    ...
    if (loadPeerMac(peerMac)) {
        ...
        pairingState = STATE_IDLE;
        applyManualChannel();
        Serial.println("[Bridge] Existing peer restored");
    } else {
        pairingState = STATE_DISC;
        lastAction = 0;
        Serial.println("[Bridge] Starting discovery");
    }
}
...
void pairingLoop() {
    ...
    if (pendingAck) {
        ...
        pendingAck = false;
        Serial.println("[Bridge] ACK sent from loop");
        addPeer(peerMac);
        savePeerMac(peerMac);
        paired = true;
        pairingState = STATE_IDLE;
        applyManualChannel();
        Serial.println("[Bridge] Pairing completed");
        return;
    }
    ...
    if (pairingState == STATE_DISC) {
        if (now - lastAction >= RETRY_MS) {
            ...
            lastAction = now;
            Serial.println("[Bridge] Sending JSP_DISC");
        }
    } else if (pairingState == STATE_WAIT_ACK) {
        if (now - phaseStart >= TIMEOUT_MS) {
            pairingState = STATE_DISC;
            memset(peerMac, 0, 6);
            Serial.println("[Bridge] Pairing timeout, restart discovery");
        }
    } 
}
