...
void phase0_lightScan() {
  for (int pin = 0; pin < MAX_PINS; pin++) {
    ...
    ((s.maxv - s.minv) < 120 && s.drift > 1000 && !(s.avg > 1650 && s.avg < 2250));
...
bool analogStable =
    ...
    r.state = PIN_ANALOG;
      ...
      else if (s.avg > 4000 && (s.maxv - s.minv) < 150) {
      r.state = PIN_DIGITAL;
...
uint8_t parsePinList(const char* str, uint8_t* output, uint8_t maxPins) {
    if (str == NULL || str[0] == '\0') return 0;
    
    uint8_t count = 0;
    const char* p = str;
    
    while (*p != '\0' && count < maxPins) {
        while (*p == ' ') p++;
...
void runPinScanIfNeeded() {
  ...
  uint8_t digList[64];
  uint8_t anaList[64];
  uint8_t digCount = parsePinList(manualDigitalPins, digList, 64);
  uint8_t anaCount = parsePinList(manualAnalogPins, anaList, 64);

  if (digCount > 0 || anaCount > 0) {
    Serial.println("Using manual pins; automatic scan bypassed.");

    prefs.putUChar(KEY_DIGITAL_COUNT, digCount);
    prefs.putBytes(KEY_DIGITAL_LIST, digList, digCount);
    prefs.putUChar(KEY_ANALOG_COUNT, anaCount);
    prefs.putBytes(KEY_ANALOG_LIST, anaList, anaCount);
    prefs.putUChar(KEY_PHASE, 2);
    prefs.end();
    Serial.printf("Saved %d digital and %d analog pins.\n", digCount, anaCount);
    return;
  }
